...
def scoreDeveloper(changeHistory, queryFuncs, halfLifeDays, modWeight, callWeight):
    totalScore = 0.0
...
parser.add_argument("--half-life",   type=float, default=90)
...
for email, changeHistory in expertiseMap.items():
    score, expertise = scoreDeveloper(changeHistory, queryFuncs, args.half_life, args.mod_weight, args.call_weight)
    if score > 0:
